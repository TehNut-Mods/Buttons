	...
	public static class Handler implements IMessageHandler<MessageDeleteItem, IMessage> {
		...
		public IMessage onMessage(MessageDeleteItem message, MessageContext ctx) {
			...
			if (message.isFindAll()) {
				...
				JustEnoughWidgets.LOGGER.info("{} used the {} button to delete all items matching {}", player.getDisplayNameString(), "justenoughwidgets:button_delete", message.getStack().getDisplayName());
			} else {
				JustEnoughWidgets.LOGGER.info("{} used the {} button to delete {}", player.getDisplayNameString(), "justenoughwidgets:button_delete", message.getStack().getDisplayName());
			}
